...
class FacturationCovage extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'export:FacturationCovage';
     ...
     * @var string
     */
    protected $description = 'Récupération des données pour la facturation de Covage';
        ...
        try {
            $query = "
                SELECT 
                    ...
                    t9.reference_prestation_prise as 'RefCmdCovage',
            ...
            $facturationCovage = DB::select($query, $params);   
            
            $fileName = "EXPORT_FacturationCovage_".date('Ymd_His').".csv";
            $filePath = $_ENV['EXPORT_FOLDER'] . '/'. $fileName;

            $file = fopen($filePath, 'w');
            fputcsv($file, Facturation::$Covage['column']);

            foreach($facturationCovage as $facturation) {
                fputcsv($file, array(
